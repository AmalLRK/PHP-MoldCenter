...
class Office365_Client
{
    ...
    public function createAuthUrl() {
        global $apiConfig;
        $query_params = array ('response_type' => 'code','client_id' => $apiConfig ['oauth2_client_id'],'client_secret' => $apiConfig ['oauth2_secret'],'redirect_uri' => $apiConfig ['oauth2_redirect'],'scope' => $apiConfig ['scope'],'state' => $apiConfig ['state']
        );
    ...
    public function fetchUserDatas($token)
    {
        $req = new HttpPost('https://graph.microsoft.com/beta/me');
        
        $req->setAccessToken($this->accessToken ? $this->accessToken : $token);
        $req->send();
        $responseObj = json_decode($req->getHttpResponse());
        return $responseObj;
    }

    public function fetchUserPicture($token)
    {
        $req = new HttpPost('https://graph.microsoft.com/beta/me/photo/$value');
        
        $req->setAccessToken($this->accessToken ? $this->accessToken : $token);
        $req->send();
        $responseObj = $req->getHttpResponse();
        return $responseObj;
    }
